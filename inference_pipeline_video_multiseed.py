...
# prompt = [
#     "a detailed photo of the end of the universe, artstation, vaporwave, neon", 
#     "a detailed photo of the end of the universe with a blackhole, artstation, vaporwave, neon"]
prompt = [
    "A ultradetailed beautiful panting of league of legends concept art of a monster, comic book outline, trending on artstation, vaporwave, by conrad roset", 
    "A ultradetailed beautiful panting of league of legends concept art of a monster, comic book outline, trending on artstation, vaporwave, by conrad roset"]


save_dir = "results_test/test_5"
name_prefix = "test"
...
for img_idx, interp_factor in enumerate(interp_factors):
    ...
    generator_1 = torch.manual_seed(0)    # Seed generator to create the inital latent noise
    ...
    latents_1 = torch.randn(
        (batch_size, unet.in_channels, height // 8, width // 8),
        generator=generator_1,
    )
    generator_2 = torch.manual_seed(10)    # Seed generator to create the inital latent noise

    latents_2 = torch.randn(
        (batch_size, unet.in_channels, height // 8, width // 8),
        generator=generator_2,
    )

    print("latents_1", latents_1.max(), latents_1.min(), latents_1.mean(), latents_1.std())
    print("latents_2", latents_2.max(), latents_2.min(), latents_2.mean(), latents_2.std())

    
    # Remove mean/std
    latents = interp_factor * latents_1 + (1 - interp_factor) * latents_2
    latents = (latents - latents.mean()) / latents.std()
    
    mean_interp = interp_factor * latents_1.mean() + (1 - interp_factor) * latents_2.mean()
    std_interp = interp_factor * latents_1.std() + (1 - interp_factor) * latents_2.std()

    latents = latents * std_interp + mean_interp
    #print(latents)

    #latents = (latents - latents.min()) / (latents.max() - latents.min()) * (latents_1.max() - latents_1.min()) + latents_1.min()
    print("latents", latents.max(), latents.min(), latents.mean(), latents.std())
